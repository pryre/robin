...
--- | --- | --- | --- | --- | --- | ---
BOARD_REV | uint | Selects what version of the board to setup sensors for | | | | True
...
BAUD_RATE_0 | uint | Baud rate for the the COMM_0 port - set to 0 to disable | | | | True
BAUD_RATE_1 | uint | Baud rate for the the COMM_1 port - set to 0 to disable | | | | True
TIMESYNC_ALPHA | float | TODO - also check min-max values | | | | False
SYS_ID | uint | Sets the MAVLINK System ID parameter for the message header | | | | True
COMP_ID | uint | Sets the MAVLINK Component ID parameter for the message header | | | | True
...
STRM0_SRV_OUT | uint | Communication update rate for commanded servo output in Hz (set to 0 to turn off stream) | 10.0 |  | scalar | False
...
STRM1_SRV_OUT | uint | Communication update rate for commanded servo output in Hz (set to 0 to turn off stream) | 10.0 |  | scalar | False
...
CBRK_IMU | uint | Sensor update rate for barometer in Hz (TODO) | 0.0 |  | scalar | False
...
TIMEOUT_THRTL | uint | Throttle timeout in (ms) to prevent accidentally leaving armed | 10000000 |  | scalar | False
MIXER | uint | Mixer type to use (see mixer_type_t enum) | MIXER_QUADCOPTER_X |  | scalar | True
