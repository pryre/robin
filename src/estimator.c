...
static void lpf_update( v3d* accel, v3d* gyro ) {
	...
	_accel_LPF.x = fix16_sadd( fix16_smul( fix16_ssub( _fc_1, alpha_acc ), _accel_LPF.x ),
							   fix16_smul( alpha_acc, accel->x ) );
	_accel_LPF.y = fix16_sadd( fix16_smul( fix16_ssub( _fc_1, alpha_acc ), _accel_LPF.y ),
							   fix16_smul( alpha_acc, accel->y ) );
	_accel_LPF.z = fix16_sadd( fix16_smul( fix16_ssub( _fc_1, alpha_acc ), _accel_LPF.z ),
							   fix16_smul( alpha_acc, accel->z ) );

	fix16_t alpha_gyro = get_param_fix16( PARAM_GYRO_ALPHA );
	_gyro_LPF.x = fix16_sadd( fix16_smul( fix16_ssub( _fc_1, alpha_gyro ), _gyro_LPF.x ),
							  fix16_smul( alpha_gyro, gyro->x ) );
	_gyro_LPF.y = fix16_sadd( fix16_smul( fix16_ssub( _fc_1, alpha_gyro ), _gyro_LPF.y ),
							  fix16_smul( alpha_gyro, gyro->y ) );
	_gyro_LPF.z = fix16_sadd( fix16_smul( fix16_ssub( _fc_1, alpha_gyro ), _gyro_LPF.z ),
							  fix16_smul( alpha_gyro, gyro->z ) );
}

static void fuse_heading_estimate( qf16* q_est, const qf16* q_mes,
								   const fix16_t alpha ) {
	qf16 q_temp;
	qf16 dq;
	...
	qf16_mul( &dq, q_mes,
			  &q_temp ); // Difference between current estimate and measured
	...
	rot_axis.y = 0;
	rot_axis.z = _fc_1;
	qf16_from_axis_angle( &q_temp, &rot_axis, rot_a ); // Create a rotation
	// quaternion for the flat
...
//  212 | 195 us (acc and gyro only, not exp propagation no quadratic
//  integration)
static void estimator_update( uint32_t time_now, v3d* accel, v3d* gyro ) {
	...
	if ( ( get_param_uint( PARAM_EST_USE_ACC_COR ) ) && ( a_sqrd_norm < fix16_mul( fix16_sq( _fc_1_15 ), fix16_sq( _fc_gravity ) ) ) && ( a_sqrd_norm > fix16_mul( fix16_sq( _fc_0_85 ), fix16_sq( _fc_gravity ) ) ) ) {
		...
		qf16_inverse(
			&q_hat_inv,
			&q_hat ); // Need to invert to bring the vector into the body frame
		...
		// Calculate correction rates and integrate biases from accelerometer
		// feedback
	...
	if ( get_param_uint( PARAM_EST_USE_QUAD_INT ) ) {
		...
		// wbar = ((-1.0f / 12.0f) * w2) + ((8.0f / 12.0f) * w1) + ((5.0f / 12.0f) *
		// _gyro_LPF);
	...
	if ( sqrd_norm_w > 0 ) {
		...
		if ( get_param_uint( PARAM_EST_USE_MAT_EXP ) ) {
			...
			// This is can cause some serious RAM issues if either caching or lookup
			// tables are enabled
			// XXX: Even with caching turned off, this should give a good performance
			// increase (hopefully around 25%)
			fix16_t t1 = fix16_cos( fix16_div( fix16_mul( norm_w, dt ), _fc_2 ) );
			fix16_t t2 = fix16_mul( fix16_div( _fc_1, norm_w ),
									fix16_sin( fix16_div( fix16_mul( norm_w, dt ), _fc_2 ) ) );

			/*
qhat_np1.w = t1*q_hat.w   + t2*(          - p*q_hat.x - q*q_hat.y -
r*q_hat.z);
qhat_np1.x = t1*q_hat.x   + t2*(p*q_hat.w             + r*q_hat.y -
q*q_hat.z);
qhat_np1.y = t1*q_hat.y   + t2*(q*q_hat.w - r*q_hat.x             +
p*q_hat.z);
qhat_np1.z = t1*q_hat.z   + t2*(r*q_hat.w + q*q_hat.x - p*q_hat.y);
*/

			// qdot.w = t2*(((-p*q_hat.x) + (-q*q_hat.y)) + (-r*q_hat.z)
			qdot.a = fix16_mul( t2, fix16_add( fix16_add( fix16_mul( -p, q_hat.b ),
														  fix16_mul( -q, q_hat.c ) ),
											   fix16_mul( -r, q_hat.d ) ) );
			qdot.b = fix16_mul(
				t2, fix16_add( fix16_add( fix16_mul( p, q_hat.a ), fix16_mul( r, q_hat.c ) ),
							   fix16_mul( -q, q_hat.d ) ) );
			qdot.c = fix16_mul( t2, fix16_add( fix16_add( fix16_mul( q, q_hat.a ),
														  fix16_mul( -r, q_hat.b ) ),
											   fix16_mul( p, q_hat.d ) ) );
			qdot.d = fix16_mul(
				t2, fix16_add( fix16_add( fix16_mul( r, q_hat.a ), fix16_mul( q, q_hat.b ) ),
							   fix16_mul( -p, q_hat.c ) ) );
      ...
      */
			...
			qdot.a = fix16_mul( _fc_0_5, fix16_add( fix16_mul( -p, q_hat.b ),
													fix16_add( fix16_mul( -q, q_hat.c ),
															   fix16_mul( -r, q_hat.d ) ) ) );
			qdot.b = fix16_mul( _fc_0_5, fix16_add( fix16_mul( p, q_hat.a ),
													fix16_add( fix16_mul( r, q_hat.c ),
															   fix16_mul( -q, q_hat.d ) ) ) );
			qdot.c = fix16_mul( _fc_0_5, fix16_add( fix16_mul( q, q_hat.a ),
													fix16_add( fix16_mul( -r, q_hat.b ),
															   fix16_mul( p, q_hat.d ) ) ) );
			qdot.d = fix16_mul( _fc_0_5, fix16_add( fix16_mul( r, q_hat.a ),
													fix16_add( fix16_mul( q, q_hat.b ),
															   fix16_mul( -p, q_hat.c ) ) ) );
	...
	if ( ( _system_status.sensors.ext_pose.health == SYSTEM_HEALTH_OK ) && _sensors.ext_pose.status.new_data ) {
		fuse_heading_estimate( &q_hat, &_sensors.ext_pose.q,
							   get_param_fix16( PARAM_FUSE_EXT_HDG_W ) );
		...
		fuse_heading_estimate( &q_hat, &_sensors.mag.q,
							   get_param_fix16( PARAM_FUSE_MAG_HDG_W ) );
	...
	// euler_from_quat(&q_hat, &_state_estimator.phi, &_state_estimator.theta,
	// &_state_estimator.psi);

	if ( !get_param_uint( PARAM_EST_USE_ADPT_BIAS ) )
		reset_adaptive_gyro_bias(); // TODO: XXX: The adaptive bias' are good, but
	// without proper mag support, they cause
	// lasting errors if the mav is turned upside
	// down
