...
static bool communication_decode( mavlink_channel_t chan, uint8_t c ) {
	...
	if ( mavlink_parse_char( chan, c, &msg, &status ) ) {
		...
		if ( ( !get_param_uint( PARAM_STRICT_GCS_MATCH ) ) || ( ( msg.sysid == mavlink_gcs.sysid ) && ( msg.compid == mavlink_gcs.compid ) ) ) {
			...
			// switch
			if ( ( !( mavlink_get_channel_status( chan )->flags & MAVLINK_STATUS_FLAG_IN_MAVLINK1 ) ) && ( mavlink_get_proto_version( chan ) == 1 ) ) {
	...
	while ( ( system_micros() - time_start_read ) < time_read_max ) {
		...
		if ( !port0_done ) {
			if ( comms_waiting( COMM_PORT_0 ) ) {
				// port0_done = communication_decode( MAVLINK_COMM_0,
				// serialRead(Serial1) );
		...
		if ( !port1_done ) {
			if ( comms_waiting( COMM_PORT_1 ) ) {
				// port1_done = communication_decode( MAVLINK_COMM_1,
				// serialRead(Serial2) );
