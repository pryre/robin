...
void mavlink_handle_param_request_list( mavlink_channel_t chan, mavlink_message_t* msg, mavlink_status_t* status ) {
	if ( ( mavlink_msg_param_request_list_get_target_system( msg ) == mavlink_system.sysid ) && ( ( mavlink_msg_param_request_list_get_target_component( msg ) == mavlink_system.compid ) || ( mavlink_msg_param_request_list_get_target_component( msg ) == MAV_COMP_ID_ALL ) ) ) {
