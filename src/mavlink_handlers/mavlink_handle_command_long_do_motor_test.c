...
MAV_RESULT mavlink_handle_command_long_do_motor_test( mavlink_channel_t chan,
	...
	if ( !_mixer_to_use->mixer_ok ) {
		...
		mavlink_queue_broadcast_error(
			"[MIXER] Motor test requires saftety off, not armed" );
		...
		if ( ( ( motor_test_number < MIXER_NUM_MOTORS ) || ( motor_test_number == MIXER_TEST_MOTORS_ALL ) ) && ( ( motor_test_throttle > 0 ) && ( motor_test_throttle < _fc_1 ) ) && ( motor_test_timeout > 0 ) ) {
			...
			_motor_test.duration = 10000 * fix16_to_int( fix16_mul( motor_test_timeout,
																	_fc_100 ) ); // XXX: Motor test should be
			// accurate to 0.01 seconds,
			// and max out at ~1min
			_motor_test.throttle = motor_test_throttle;

			// XXX: Override sensor health as autopilot is in control
			//_system_status.sensors.pwm_control.count =
			...
			mavlink_queue_broadcast_error(
				"[MIXER] Cannot run motor test, bad test variables!" );
