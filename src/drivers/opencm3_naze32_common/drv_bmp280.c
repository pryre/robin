...
#define BARO_REG_DATA \
	0xF7 // 6 bytes of info: press_msb, press_lsb, press_xlsb, temp_msb, temp_lsb,
// temp_xlsb
...
bool bmp280Init(uint32_t i2c, int boardVersion ) {
	...
	ack = i2cReadBuffer( BARO_ADDRESS, BARO_REG_ID, 1,
						 &sig ); // Check the identification register to make sure
 ...
 * Asynchronous Method
 */
static volatile uint8_t baro_buffer[6];
static volatile int32_t* baro_data;

static void baro_read_CB( void ) {
