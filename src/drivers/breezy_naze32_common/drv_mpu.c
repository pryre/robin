...
void mpu_request_async_accel_read( int16_t* accData, volatile uint8_t* status ) {
	...
	// is finished
	i2c_queue_job( READ, MPU_ADDRESS, MPU_RA_ACCEL_XOUT_H, accel_buffer, 6, status,
				   &accel_read_CB );
}
#endif
...
void mpu_request_async_gyro_read( int16_t* gyroData, volatile uint8_t* status ) {
	gyro_data = gyroData;
	i2c_queue_job( READ, MPU_ADDRESS, MPU_RA_GYRO_XOUT_H, gyro_buffer, 6, status,
				   &gyro_read_CB );
}
...
void mpu_request_async_temp_read( volatile int16_t* tempData,
								  volatile uint8_t* status ) {
	temp_data = tempData;
	i2c_queue_job( READ, MPU_ADDRESS, MPU_RA_TEMP_OUT_A, temp_buffer, 2, status,
				   &temp_read_CB );
}
