...
#define BIND_TELEM1 "udp://0.0.0.0:14510@0.0.0.0:14511"
#define BIND_TELEM2 "udp://0.0.0.0:14512@0.0.0.0:14513"
...
bool comms_init_port( comms_port_t port ) {
	...
	case COMM_PORT_0: {
		...
		if ( udp_split( &bind_port, remote_host, &remote_port, conn_udp ) ) {
			...
			system_debug_print( "[COMMS] Failed to parse telem0 comm port: %s", BIND_TELEM1 );
		}
	...
	case COMM_PORT_1: {
		...
		if ( udp_split( &bind_port, remote_host, &remote_port, conn_udp ) ) {
			...
			system_debug_print( "[COMMS] Failed to parse telem1 comm port: %s", BIND_TELEM2 );
		}
