...
static void status_buzzer_update( uint32_t time_now ) {
	...
	if ( ( buzzer.num_beeps == 0 ) && ( ( _system_status.state == MAV_STATE_CRITICAL ) || ( _system_status.state == MAV_STATE_EMERGENCY ) ) ) {

		status_buzzer_new_beep_set( 1, 100000 );
	}

	if ( ( buzzer.num_beeps > 0 ) && ( ( time_now - buzzer.last_beep ) > buzzer.period ) ) {
