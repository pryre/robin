...
static inline void pid_reset(pid_t *pid, fix16_t prev_x) {
	pid->integrator = 0;
	pid->prev_time = 0;
	pid->prev_x = prev_x;
}

static inline void pid_set_gains(pid_t *pid, fix16_t kp, fix16_t ki, fix16_t kd, fix16_t tau) {
	pid->kp = kp;
	pid->ki = ki;
	pid->kd = kd;
	pid->tau = tau;
...
static inline void pid_set_min_max(pid_t *pid, fix16_t min, fix16_t max) {
	pid->min = min;
	pid->max = max;
}

static inline void pid_init(pid_t *pid, fix16_t kp, fix16_t ki, fix16_t kd, fix16_t tau, fix16_t initial_x, fix16_t initial_x_dot, fix16_t initial_setpoint, fix16_t initial_output, fix16_t min, fix16_t max) {
...
// Returns:
//	- Output Command
static inline fix16_t pid_step(pid_t *pid, uint32_t time_now, fix16_t sp, fix16_t x, fix16_t x_dot, bool use_x_dot) {
	fix16_t now = fix16_from_float(time_now * (1e-6));	//Time in seconds
	fix16_t dt = fix16_sub(now, pid->prev_time);
	pid->prev_time = now;
