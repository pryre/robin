...
typedef enum { AXIS_LOCK_X,
			   AXIS_LOCK_Y,
			   AXIS_LOCK_Z } qf16_axis_lock_t;

static const qf16 NED_ENU_Q = {0, _fc_sqrt_0_5, -_fc_sqrt_0_5, 0};
// static const qf16 NED_IMU_Q = {0, _fc_1, 0, 0};

static inline fix16_t fix16_constrain( fix16_t i, const fix16_t min,
									   const fix16_t max ) {
...
static inline fix16_t qf16_dot_full( const qf16* q1, const qf16* q2 ) {
	return fix16_add(
		fix16_add( fix16_add( fix16_mul( q1->a, q2->a ), fix16_mul( q1->b, q2->b ) ),
				   fix16_mul( q1->c, q2->c ) ),
		fix16_mul( q1->d, q2->d ) );
}
...
static inline void qf16_dcm_z( v3d* b_z, const qf16* q ) {
	...
	b_z->z = fix16_add(
		fix16_sq( q->a ),
		fix16_add( -fix16_sq( q->b ), fix16_add( -fix16_sq( q->c ), fix16_sq( q->d ) ) ) );
}

// Returns the rotation between two vectors
// Method used derrived from PX4/Matrix
static inline void qf16_from_shortest_path( qf16* dest, const v3d* v1,
											const v3d* v2 ) {
	v3d cr;
	v3d_cross( &cr, v1, v2 );
	...
	if ( ( v3d_norm( &cr ) <= _fc_eps ) && ( dt < 0 ) ) {
		...
		dest->a = fix16_add( dt, fix16_sqrt( fix16_mul( fix16_sq( v3d_norm( v1 ) ),
														fix16_sq( v3d_norm( v2 ) ) ) ) );
	}
...
static inline void euler_from_quat( qf16* q, fix16_t* phi, fix16_t* theta,
									fix16_t* psi ) {
	*phi = fix16_atan2(
		fix16_mul( _fc_2, fix16_add( fix16_mul( q->a, q->b ), fix16_mul( q->c, q->d ) ) ),
		fix16_sub( _fc_1, fix16_mul( _fc_2, fix16_add( fix16_mul( q->b, q->b ),
													   fix16_mul( q->c, q->c ) ) ) ) );

	*theta = fix16_asin( fix16_mul(
		_fc_2, fix16_sub( fix16_mul( q->a, q->c ), fix16_mul( q->d, q->b ) ) ) );

	*psi = fix16_atan2(
		fix16_mul( _fc_2, fix16_add( fix16_mul( q->a, q->d ), fix16_mul( q->b, q->c ) ) ),
		fix16_sub( _fc_1, fix16_mul( _fc_2, fix16_add( fix16_mul( q->c, q->c ),
													   fix16_mul( q->d, q->d ) ) ) ) );
}

static inline void quat_from_euler( qf16* q, fix16_t phi, fix16_t theta,
									fix16_t psi ) {
	...
	q->a = fix16_add( fix16_mul( t0, fix16_mul( t2, t4 ) ),
					  fix16_mul( t1, fix16_mul( t3, t5 ) ) );
	q->b = fix16_sub( fix16_mul( t0, fix16_mul( t3, t4 ) ),
					  fix16_mul( t1, fix16_mul( t2, t5 ) ) );
	q->c = fix16_add( fix16_mul( t0, fix16_mul( t2, t5 ) ),
					  fix16_mul( t1, fix16_mul( t3, t4 ) ) );
	q->d = fix16_sub( fix16_mul( t1, fix16_mul( t2, t4 ) ),
					  fix16_mul( t0, fix16_mul( t3, t5 ) ) );
}
...
static inline void matrix_to_qf16( qf16* dest, const mf16* mat ) {
	...
	if ( trace > 0 ) {
		...
		fix16_t s = fix16_sqrt( fix16_sub(
			mat->data[i][i],
			fix16_sub( mat->data[j][j], fix16_add( mat->data[k][k], _fc_1 ) ) ) );
...
static inline void dcm_from_basis( mf16* dcm, const v3d* b_x, const v3d* b_y,
								   const v3d* b_z ) {
	dcm->data[0][0] = b_x->x;
	dcm->data[0][1] = b_x->y;
	dcm->data[0][2] = b_x->z;
...
static inline void qf16_align_to_axis( qf16* dest, const qf16* input,
									   const qf16* reference,
									   const qf16_axis_lock_t axis ) {
	qf16 q_temp;
	qf16 dq;
	...
	qf16_rotate( &fv, &dq,
				 &fv ); // Rotate the flat vector by the difference in rotation
	...
	qf16 q_rot;
	qf16_from_axis_angle(
		&q_rot, &rot_axis,
		rot_a );					 // Create a rotation quaternion for the flat rotation the axis
